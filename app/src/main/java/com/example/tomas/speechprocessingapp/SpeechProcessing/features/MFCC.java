...
public class MFCC
{
    ...
    /*
    private float[] timefreq(float[] sig)
    {

        sigproc SigProc = new sigproc();
        //Apply hamming window to the signal
        float[] hamm = new float[sig.length + 1];
        Arrays.fill(hamm, 1);
        hamm = SigProc.makeWindow(hamm, 0);
        //NormL2 of hamming win
        int ind = hamm.length / 2;
        float temp = (float) Math.pow(hamm[ind], 2);
        for (int i = (ind + 1); i < hamm.length; i++) {
            float temp2 = ((float) Math.pow(hamm[i], 2));
            ;
            temp = temp + temp2;
        }
        float L2hamm = (float) Math.sqrt(temp);
        //Compute Hamming
        float sig_fft[] = SigProc.makeWindow(sig, 0);
        //Time-Frequency representation
        int[] tau = new int[sig.length / 2];
        float[] tfr = new float[sig.length];
        for (int i = 0; i < (tfr.length / 2); i++) {
            tfr[i] = sig_fft[i] * hamm[tau.length + i] / L2hamm;
        }
        tfr = SigProc.signal_fft(tfr, (int) sampleRate);
        //tfr = SigProc.signal_fft(sig, (int) sampleRate);
        //Periodogram
        for (int i = 0; i < (tfr.length); i++) {
            tfr[i] = (float) Math.pow(abs(tfr[i]), 2);
        }
        return tfr;
    }
    */
